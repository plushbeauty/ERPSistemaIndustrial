create table if not exists public.erp_fmea (
 ...
 codigo text not null, processo text not null, etapa text not null default '', falha text not null, efeito text not null default '', causa text not null default '', controle text not null default '',
 severidade integer not null check(severidade between 1 and 10), ocorrencia integer not null check(ocorrencia between 1 and 10), deteccao integer not null check(deteccao between 1 and 10),
 rpn integer generated always as (severidade*ocorrencia*deteccao) stored,
 acao text, responsavel text, data_limite date, status text not null default 'ABERTO',
