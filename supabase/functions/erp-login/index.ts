...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!empresa || !identificador || !senha) return json(origin, { error: 'Informe empresa, usuário e senha.' }, 400)

    const serviceRole = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')
    const anonKey = Deno.env.get('SUPABASE_ANON_KEY')
    const supabaseUrl = Deno.env.get('SUPABASE_URL')
    if (!serviceRole || !anonKey || !supabaseUrl) return json(origin, { error: 'Ambiente de autenticação não configurado.' }, 500)
    ...
    const { data: empresas, error: empresaError } = await admin
      ...
      .limit(20)
    if (empresaError) return json(origin, { error: 'Falha ao localizar a empresa.' }, 500)
    ...
    const { data: usuarios, error: usuarioError } = await admin
      ...
      .is('deleted_at', null)
      .limit(500)
    if (usuarioError) return json(origin, { error: 'Falha ao localizar o usuário do ERP.' }, 500)
    ...
    if (!usuario.auth_user_id) return json(origin, { error: 'Usuário ainda não foi provisionado para autenticação.' }, 403)

    const { data: authUserData, error: authUserError } = await admin.auth.admin.getUserById(usuario.auth_user_id)
    if (authUserError || !authUserData.user) return json(origin, { error: 'Vínculo de autenticação inválido.' }, 403)
    ...
    const { data: authData, error: authError } = await authClient.auth.signInWithPassword({ email: authEmail, password: senha })
    if (authError || !authData.session || !authData.user) return json(origin, { error: 'Usuário ou senha inválidos.' }, 401)

    if (Number(usuario.nivel_admin) === 1 && empresaEncontrada.plano_status !== 'ativo' && empresaEncontrada.trial_ends_at && Date.now() >= new Date(empresaEncontrada.trial_ends_at).getTime()) {
    ...
    if (usuario.setor_id) {
      const { data: setor } = await admin.from('erp_setores').select('codigo,nome').eq('id', usuario.setor_id).eq('empresa_id', empresaEncontrada.id).maybeSingle()
