...
function renderFatal(title: string, message: string) {
  mountNode.innerHTML = `<main class="bootstrap-fatal"><section><div class="bootstrap-eyebrow">SGQ ERP INDUSTRIAL</div><h1>${title}</h1><p>${message}</p><button onclick="location.reload()">Tentar novamente</button></section></main>`
}
...
async function bootstrap() {
  ...
  try {
    const [entry, help, actions, boundary, pwa, theme, sidebar, header, backdrop] = await Promise.all([
      import('./AppEntryV2'), import('./GlobalHelp'), import('./components/ERPHeaderActions'), import('./components/GlobalErrorBoundary'),
      import('./components/PwaInstallButton'), import('./context/ThemeContext'), import('./context/SidebarContext'), import('./layout/AppHeader'), import('./layout/Backdrop'),
    ])
    ...
    ReactDOM.createRoot(mountNode).render(
      <React.StrictMode><GlobalErrorBoundary><ThemeProvider><SidebarProvider>
        <AppEntry />
        {shell && <AppHeader />}
        {shell && <Backdrop />}
        ...
        {shell && <TabletHost />}
      </SidebarProvider></ThemeProvider></GlobalErrorBoundary></React.StrictMode>,
    )
  } catch (error) {
