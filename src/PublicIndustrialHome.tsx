...
const modules = [
  ['PCP e Produção', 'Planejamento, ordens, sequenciamento e acompanhamento do chão de fábrica.', '/modulos/pcp', Factory],
  ['Estoque e Materiais', 'Saldos, movimentações, rastreabilidade e necessidades de materiais.', '/modulos/estoque', Boxes],
  ['Recebimento de Materiais', 'Importe o XML da NF-e, confira os itens e distribua quantidades por múltiplos lotes.', '/modulos/recebimento', PackageCheck],
  ['Qualidade', 'RPNC, auditorias, ações, documentos e rastreabilidade da qualidade.', '/modulos/qualidade', ClipboardCheck],
  ['Manutenção', 'Máquinas, manutenção preventiva, corretiva e histórico operacional.', '/modulos/manutencao', Wrench],
  ['Financeiro e Fiscal', 'Visão financeira e preparação dos processos fiscais do ERP.', '/modulos/fiscal', Receipt],
  ['Indicadores', 'Acompanhe produção, qualidade, estoque e desempenho com dados do sistema.', '/modulos/indicadores', BarChart3],
] as const
...
export default function PublicIndustrialHome() {
  return <div className="public-industrial home-v2">
    ...
    <main>
      ...
      <section id="modulos" className="public-section"><div className="section-heading"><span className="public-kicker">MÓDULOS</span><h2>O ERP acompanha a operação real da fábrica.</h2><p>Cada card abre primeiro uma página de apresentação com finalidade, fluxo, indicadores e integração. A entrada no módulo operacional continua protegida por autenticação e permissões.</p></div><div className="module-cards">{modules.map(([title,desc,href,Icon]) => <article className="module-card resource-card" key={title}><div className="module-icon"><Icon size={22}/></div><h3>{title}</h3><p>{desc}</p><a className="public-link" href={href}>Conhecer módulo <ArrowRight size={15}/></a></article>)}</div></section>
