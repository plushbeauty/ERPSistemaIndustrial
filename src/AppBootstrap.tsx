...
class BootstrapBoundary extends Component<{children:ReactNode},{error:Error|null}>{
  ...
  render(){
    if(this.state.error)return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',padding:24,background:'#07141a',fontFamily:'Inter,system-ui,sans-serif',color:'#fff'}}><section style={{width:'min(680px,100%)',border:'1px solid rgba(255,255,255,.14)',borderRadius:24,padding:28,background:'linear-gradient(145deg,#102a34,#08191f)',boxShadow:'0 30px 90px rgba(0,0,0,.35)'}}><div style={{fontSize:11,fontWeight:900,letterSpacing:'.18em',color:'#d6b36a'}}>SGQ ERP INDUSTRIAL</div><h1 style={{fontSize:28,margin:'10px 0 8px'}}>O ambiente encontrou uma falha ao iniciar</h1><p style={{color:'rgba(255,255,255,.7)',lineHeight:1.7,margin:0}}>A inicialização falhou. Esta tela de recuperação evita o antigo estado azul/blank e permite reiniciar o bootstrap.</p><pre style={{whiteSpace:'pre-wrap',wordBreak:'break-word',marginTop:18,padding:16,borderRadius:14,background:'rgba(0,0,0,.25)',color:'#f1d89e',fontSize:12}}>{this.state.error.message}</pre><div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:18}}><button type="button" onClick={()=>location.reload()} style={{border:0,borderRadius:12,padding:'12px 18px',fontWeight:900,cursor:'pointer'}}>Recarregar ambiente</button><button type="button" onClick={()=>{void supabase.auth.signOut();location.replace('/login')}} style={{border:'1px solid rgba(255,255,255,.18)',borderRadius:12,padding:'12px 18px',fontWeight:900,cursor:'pointer',background:'transparent',color:'#fff'}}>Limpar sessão e entrar</button><a href="/" style={{border:'1px solid rgba(255,255,255,.18)',borderRadius:12,padding:'12px 18px',fontWeight:900,color:'#fff',textDecoration:'none'}}>Voltar ao site</a></div></section></main>
...
function AccessGate({children}:{children:ReactNode}){
  ...
  useEffect(()=>{let alive=true;void(async()=>{
    try{
      ...
      if(!data.session?.user){if(alive)setState('denied');return}
  ...
  if(state==='denied'){
    ...
    return <div role="status" aria-live="polite" style={{minHeight:'100vh',display:'grid',placeItems:'center'}}>Redirecionando para o login…</div>
  }
  return <>{children}</>
}
