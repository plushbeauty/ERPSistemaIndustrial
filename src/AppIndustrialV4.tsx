...
import { Activity, BarChart3, Boxes, Building2, CalendarDays, ClipboardCheck, Factory, LayoutGrid, Package, Plus, Search, Settings, ShoppingCart, Trash2, Truck, Users, Wallet, Wrench, X } from 'lucide-react'
...
const modules: Module[] = [
  M('Produtos', 'Produtos e materiais', 'Código, nome, grupo, unidade, estoque, custos e venda.', Package, 'Cadastros', 'erp_produtos', [
    { key: 'codigo', label: 'Código do produto', required: true }, { key: 'nome', label: 'Nome', required: true }, { key: 'unidade', label: 'Unidade' }, { key: 'tipo', label: 'Tipo / grupo' }, { key: 'estoque_minimo', label: 'Estoque mínimo', type: 'number' }, { key: 'custo_medio', label: 'Custo médio', type: 'number' }, { key: 'preco_venda', label: 'Preço de venda', type: 'number' },
  ]),
  M('Clientes', 'Clientes', 'Cadastro comercial e documentos dos clientes.', Users, 'Cadastros', 'erp_clientes', [
    { key: 'nome', label: 'Nome / Razão social', required: true }, { key: 'documento', label: 'CPF / CNPJ' }, { key: 'email', label: 'E-mail', type: 'email' }, { key: 'telefone', label: 'Telefone' },
  ]),
  M('Fornecedores', 'Fornecedores', 'Cadastro de fornecedores e documentos.', Truck, 'Cadastros', 'erp_fornecedores', [
    { key: 'razao_social', label: 'Razão social', required: true }, { key: 'nome_fantasia', label: 'Nome fantasia' }, { key: 'documento', label: 'CNPJ / CPF' }, { key: 'email', label: 'E-mail', type: 'email' }, { key: 'telefone', label: 'Telefone' },
  ]),
  M('Setores', 'Setores', 'Estrutura organizacional e centros operacionais.', Building2, 'Cadastros', 'erp_setores', [
    { key: 'codigo', label: 'Código', required: true }, { key: 'nome', label: 'Nome', required: true },
  ]),
  M('Máquinas', 'Máquinas e equipamentos', 'Ativos industriais, identificação e situação.', Wrench, 'Cadastros', 'erp_maquinas', [
    { key: 'codigo', label: 'Código', required: true }, { key: 'nome', label: 'Nome', required: true }, { key: 'tipo', label: 'Tipo', required: true }, { key: 'fabricante', label: 'Fabricante' }, { key: 'modelo', label: 'Modelo' }, { key: 'status', label: 'Status' },
  ]),
  M('Engenharia / BOM', 'Engenharia e BOM', 'Ficha técnica, versões e estrutura dos produtos.', Boxes, 'Produção', 'erp_fichas_tecnicas', [
    { key: 'produto_id', label: 'Produto ID', required: true }, { key: 'versao', label: 'Versão', type: 'number', required: true }, { key: 'rendimento', label: 'Rendimento', type: 'number' }, { key: 'unidade_rendimento', label: 'Unidade' }, { key: 'observacoes', label: 'Observações' },
  ]),
  M('MRP e Planejamento', 'MRP e planejamento', 'Parâmetros, necessidades e planejamento de materiais.', Activity, 'Produção', 'erp_parametros_processo', [
    { key: 'processo_id', label: 'Processo ID', required: true }, { key: 'grupo', label: 'Grupo' }, { key: 'codigo', label: 'Código', required: true }, { key: 'nome', label: 'Nome', required: true }, { key: 'tipo', label: 'Tipo' }, { key: 'unidade', label: 'Unidade' }, { key: 'valor_padrao', label: 'Valor padrão' },
  ]),
  M('Produção / OP', 'Ordens de produção', 'OP, planejamento, execução e apontamentos.', Factory, 'Produção', 'erp_ordens_producao', [
    { key: 'produto_id', label: 'Produto ID', required: true }, { key: 'quantidade_planejada', label: 'Quantidade', type: 'number', required: true }, { key: 'status', label: 'Status', required: true }, { key: 'data_prevista', label: 'Data prevista', type: 'date' }, { key: 'observacoes', label: 'Observações' },
  ]),
  M('PCP / Máquinas', 'Agenda de máquinas', 'Sequenciamento de operações e capacidade.', CalendarDays, 'Produção', 'erp_ordem_processos', [
    { key: 'ordem_producao_id', label: 'OP ID', required: true }, { key: 'processo_id', label: 'Processo ID', required: true }, { key: 'sequencia', label: 'Sequência', type: 'number', required: true }, { key: 'maquina_id', label: 'Máquina ID' }, { key: 'status', label: 'Status' },
  ]),
  M('Qualidade / QMS', 'Qualidade / RPNC', 'Inspeções, não conformidades, ações e RPN.', ClipboardCheck, 'Qualidade', 'erp_nao_conformidades', [
    { key: 'origem', label: 'Origem', required: true }, { key: 'severidade', label: 'Severidade', required: true }, { key: 'descricao', label: 'Descrição', required: true }, { key: 'status', label: 'Status', required: true }, { key: 'prazo', label: 'Prazo', type: 'date' }, { key: 'rpn', label: 'RPN', type: 'number' },
  ]),
  M('Indicadores / OEE', 'OEE e indicadores', 'Disponibilidade, performance, qualidade e paradas.', Activity, 'Gestão', 'erp_apontamentos_producao', [
    { key: 'ordem_producao_id', label: 'OP ID', required: true }, { key: 'turno', label: 'Turno' }, { key: 'quantidade_boa', label: 'Quantidade boa', type: 'number' }, { key: 'quantidade_refugo', label: 'Refugo', type: 'number' }, { key: 'parada_min', label: 'Parada (min)', type: 'number' },
  ]),
]
...
export default function AppIndustrialV4() {
...
function Dashboard({ setActive }: { setActive: (name: string) => void }) {
  ...
  return <div className="dashboard-v2">
    ...
    <div className="dashboard-metrics">
      {[['Produtos', metrics.produtos, Package, 'Produtos'], ['Clientes', metrics.clientes, Users, 'Clientes'], ['Ordens de produção', metrics.ops, Factory, 'Produção / OP'], ['RPNC', metrics.rpnc, ClipboardCheck, 'Qualidade / QMS']].map(([label, value, Icon, target]) => <button type="button" className="dashboard-metric" key={String(label)} onClick={() => setActive(String(target))}><Icon as any size={22}/><span>{label}</span><strong>{value}</strong></button>)}
    </div>
