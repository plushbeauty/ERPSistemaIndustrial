...
export default function IndustrialLoginDirect({ returnTo }: Props) {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      if (!alive) return
      if (data.session?.user) window.location.replace(safeReturnTo(returnTo))
  ...
  async function submit(event: FormEvent) {
    ...
    if (!supabaseConfigurado) {
      setError('O ambiente do ERP não está configurado. Verifique a URL e a chave pública do Supabase na Vercel.')
      return
    }
    ...
    try {
      ...
      const metadataCompany = typeof metadata.empresa_id === 'string' ? metadata.empresa_id.trim() : ''
      const metadataRole = typeof metadata.role === 'string' ? metadata.role.trim().toUpperCase() : ''
      const isMaster = ['MASTER', 'MASTER_ADMIN', 'SUPER_ADMIN'].includes(metadataRole)
      ...
      if (!profile && !isMaster) throw new Error('Usuário autenticado, mas sem cadastro correspondente no ERP.')
      const companyId = String(profile?.empresa_id ?? metadataCompany).trim()
      if (!companyId && !isMaster) throw new Error('Usuário autenticado sem empresa vinculada.')

      const { data: verified } = await supabase.auth.getSession()
      if (!verified.session?.user) throw new Error('A sessão não ficou disponível após o login.')
      setNotice('Acesso validado. Abrindo o ERP…')
  ...
  return <main className="auth-screen">
    <section className="auth-visual" aria-label="SGQ ERP Industrial">
      ...
      <div className="auth-visual-content">
        ...
        <div className="auth-visual-message">
          ...
          <div className="auth-trust"><b><ShieldCheck size={14}/> Sessão segura</b><b><Building2 size={14}/> Multiempresa</b></div>
        </div>
    ...
    <section className="auth-panel">
      <div className="auth-panel-inner">
        ...
        <p className="auth-description">Use as credenciais cadastradas. A empresa e as permissões são verificadas depois da autenticação.</p>
        <form className="auth-form" onSubmit={submit}>
          <label htmlFor="erp-direct-email">E-mail corporativo</label>
          <div className="auth-input-wrap"><input id="erp-direct-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" autoComplete="username" autoFocus required /></div>
          <div className="auth-label-row"><label htmlFor="erp-direct-password">Senha</label><button type="button" className="auth-text-button" onClick={recover} disabled={busy}>Esqueci minha senha</button></div>
          <div className="auth-input-wrap"><input id="erp-direct-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Digite sua senha" autoComplete="current-password" required /><button type="button" className="auth-password-toggle" aria-label={showPassword ? 'Ocultar senha' : 'Mostrar senha'} onClick={() => setShowPassword((value) => !value)}>{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button><KeyRound className="auth-key-icon" size={16} aria-hidden="true" /></div>
        ...
        <div className="auth-security-note"><ShieldCheck size={16}/><span>Autenticação feita pelo Supabase Auth. Nenhuma senha é armazenada no navegador.</span></div>
