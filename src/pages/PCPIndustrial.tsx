...
import PCPMonthlyPlanner from '../components/pcp/PCPMonthlyPlanner'
...
function Modal({title,onClose,children}:{title:string;onClose:()=>void;children:ReactNode}){
 return <div className="pcp-modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><div className="pcp-modal" role="dialog" aria-modal="true" aria-labelledby="pcp-modal-title"><div className="pcp-modal-head"><h2 id="pcp-modal-title">{title}</h2><button className="pcp-modal-close" onClick={onClose} aria-label="Fechar"><InlineIcon name="X" size={20}/></button></div>{children}</div></div>
}
...
function Table({title,cols,rows,search,setSearch}:{title:string;cols:string[];rows:(string|number)[][];search:string;setSearch:(v:string)=>void}){
 return <section className="crud-list"><div className="crud-list-head"><div><strong>{title}</strong><small>{rows.length} registros</small></div><div className="module-search"><InlineIcon name="Search" size={16}/><label className="sr-only" htmlFor="pcp-table-search">Pesquisar registros</label><input id="pcp-table-search" aria-label="Pesquisar registros" placeholder="Pesquisar..." value={search} onChange={e=>setSearch(e.target.value)}/></div></div><div className="crud-table-wrap"><table><thead><tr>{cols.map(c=><th key={c} scope="col">{c}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i}>{r.map((v,j)=><td key={j}>{String(v??'—')}</td>)}</tr>)}{!rows.length&&<tr><td colSpan={cols.length} className="crud-empty">Nenhum registro encontrado.</td></tr>}</tbody></table></div></section>
}
