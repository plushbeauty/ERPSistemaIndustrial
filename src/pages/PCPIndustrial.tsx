...
function ProgramacaoTimeline({programs,machines,molds,employees,onProgram}:{programs:ProgramView[];machines:Machine[];molds:Molde[];employees:Employee[];onProgram:(opId?:string)=>void}){
 ...
 const barStyle=(status:string)=>{const s=status.toLowerCase();if(s.includes('produção'))return {background:'#dcfce7',borderColor:'#10b981',color:'#166534'};if(s.includes('setup'))return {background:'#fef3c7',borderColor:'#f59e0b',color:'#92400e'};if(s.includes('parad')||s.includes('manut'))return {background:'#fee2e2',borderColor:'#ef4444',color:'#991b1b'};if(s.includes('cancel'))return {background:'#f1f5f9',borderColor:'#64748b',color:'#475569'};if(s.includes('atras'))return {background:'#ffedd5',borderColor:'#f97316',color:'#9a3412'};return {background:'#dbeafe',borderColor:'#2563eb',color:'#1e3a8a'}}
 ...
 return <section className="pcp-timeline-wrap">
  ...
  <div className="pcp-planning-strip"><div><strong>Mapa mensal de carga e sequenciamento.</strong><span>Máquinas × dias • quantidade • ciclo • cavidades • setup • turnos • eficiência • molde • operadores.</span></div><b>{base.toLocaleDateString('pt-BR',{month:'long',year:'numeric'})}</b><b>{active.reduce((s,p)=>s+Number(p.quantidade_planejada||0),0).toLocaleString('pt-BR')} peças</b><b>{new Set(active.map(p=>p.molde_id).filter(Boolean)).size} moldes</b><b>{active.length} blocos</b></div>
