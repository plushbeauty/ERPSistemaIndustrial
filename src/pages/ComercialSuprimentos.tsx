...
export default function ComercialSuprimentos(){
 ...
 async function reserveOrder(order:OrderRow){if(!order.item){setError('Pedido sem item.');return}setBusy(true);setError('');try{const e=await supabase.rpc('erp_current_empresa_id');if(e.error||!e.data)throw e.error??new Error('Empresa não identificada.');const id=String(e.data);const r=await supabase.from('erp_estoque_reservas').insert({empresa_id:id,pedido_venda_id:order.id,pedido_item_id:order.item.id,produto_id:order.item.produto_id,quantidade:Number(order.item.quantidade)});if(r.error)throw r.error;setMessage('Estoque reservado para PV-'+order.numero+'.');await load()}catch(e){setError(e instanceof Error?e.message:'Falha ao reservar estoque.')}finally{setBusy(false)}}
 ...
 return <main className="erp-page-v3 commercial-compact">
  ...
  {area==='pendentes'&&<section className="erp-card-v3 compact-card"><div className="section-title-line"><div><span>CARTEIRA COMERCIAL</span><h2>Pedidos Pendentes</h2></div><button className="erp-btn-primary" onClick={()=>setArea('pedido')}><Plus size={16}/> Novo Pedido</button></div>
   ...
   <div className="erp-table-scroll pending-table"><table className="erp-table-v3"><thead><tr><th>Pedido</th><th>Cliente</th><th>Nº Interno</th><th>Nº Peça</th><th>Nº Peça Cliente</th><th>Descrição</th><th>Dt.Entrada</th><th>Dt.Saída</th><th>Qtd.</th><th>Unid.</th><th>Qtd.Estoque</th><th>Disponibilidade</th><th>Reserva</th><th>Produção</th><th>PCP</th><th>Status</th><th>Saída NF</th><th>Ação</th></tr></thead><tbody>{filtered.map(o=>{const c=client(o.cliente_id);const p=products.find(x=>x.id===o.item?.produto_id);const stock=Number(p?.estoque_atual??0);const qty=Number(o.item?.quantidade??0);const available=stock>=qty;const due=o.data_entrega_prometida?new Date(o.data_entrega_prometida+'T23:59:59'):null;const status=String(o.status).toLowerCase().includes('entreg')?'ENTREGUE':due&&due.getTime()<Date.now()?'ATRASADO':String(o.status).toLowerCase().includes('produ')?'EM PRODUÇÃO':'PENDENTE';return <tr key={o.id}><td><b>PV-{o.numero}</b></td><td>{c?.nome||'—'}</td><td>{o.item?.codigo||'—'}</td><td>{o.item?.codigo||'—'}</td><td>—</td><td>{o.item?.descricao||'—'}</td><td>{new Date(o.created_at).toLocaleDateString('pt-BR')}</td><td>{o.data_entrega_prometida?new Date(o.data_entrega_prometida+'T12:00:00').toLocaleDateString('pt-BR'):'—'}</td><td>{o.item?.quantidade??'—'}</td><td>{p?.unidade||'—'}</td><td>{stock}</td><td><span className={available?'flow-badge done':'flow-badge pending'}>{available?'DISPONÍVEL':'SEM ESTOQUE'}</span></td><td>{available?<button className="erp-btn-secondary" title="Reservar estoque" onClick={()=>void reserveOrder(o)}>Reservar</button>:'—'}</td><td>{available?'—':<button className="erp-btn-secondary" title="Solicitar produção" onClick={()=>void sendToPCP(o)}><Factory size={13}/> Solicitar</button>}</td><td><StatusBadge kind="pcp" value={o.pcp}/></td><td><span className={status==='ENTREGUE'?'flow-badge done':status==='EM PRODUÇÃO'?'flow-badge active':'flow-badge pending'}>{status}</span></td><td><StatusBadge kind="fiscal" value={String(o.status).toLowerCase().includes('fatur')?'EMITIDA':'AGUARDANDO'}/></td><td><button className="erp-btn-secondary" title="Consultar pedido"><Eye size={14}/></button></td></tr>})}{!filtered.length&&<tr><td colSpan={18} className="crud-empty">Nenhum pedido pendente encontrado.</td></tr>}</tbody></table></div>
