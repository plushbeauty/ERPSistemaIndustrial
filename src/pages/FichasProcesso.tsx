...
export default function FichasProcesso(){
 ...
 async function save(){
  ...
  try{
   ...
   await supabase.from('erp_logs_sistema').insert({empresa_id:String(profile.data.empresa_id),usuario_id:profile.data.id,modulo:'Engenharia',acao:'FICHA_PROCESSO_SALVA',entidade:'erp_fichas_tecnicas',entidade_id:id,dados:{status:form.status,operacoes:rows.length}})
 ...
 async function advance(){if(!form.id||!nextStatus[form.status])return;const status=nextStatus[form.status];update('status',status);setTimeout(()=>void save(),0)}
