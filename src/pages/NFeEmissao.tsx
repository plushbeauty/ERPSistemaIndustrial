 ...
 * Data/Hora: 24/09/2026 - 12:51 BRT
 * Desenvolvedor: Homologado por FernandoSch.
 * ID da Revisão: REV-045
 * Alterações: Correção crítica do array horizons na linha 230, fechamento estrito
 *            de todas as chaves JSX ('main', 'div', 'nav') e integração do Bloco K.
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <nav className="flex bg-slate-50 border-b border-slate-200">
          {[
            ...
            <button key={id} className={`flex-1 px-4 py-3.5 text-base font-bold border-b-2 flex items-center justify-center gap-2 ${tab === id ? 'bg-white border-[#2563eb] text-[#2563eb]' : 'border-transparent text-slate-500'}`} onClick={() => setTab(id as any)}>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-base">
                ...
                <label className="font-semibold block">Inscrição Estadual
                  <input className="mt-1 w-full bg-slate-50 border p-2 rounded-lg outline-none" value={destinatario.inscricao_estadual} readOnly />
                </label>
              </div>
