...
export default function OperacaoIndustrial() {
  ...
  async function save(table: string, payload: Record<string, unknown>, id?: string) {
    ...
    try {
      ...
      const response = id ? await supabase.from(table).update(clean).eq('id', id).eq('empresa_id', empresaId) : await supabase.from(table).insert(clean)
  ...
  async function remove(table: string, id: string) {
    ...
    setBusy(true); setError('')
    try { const { error: e } = await supabase.from(table).delete().eq('id', id).eq('empresa_id', empresaId); if (e) throw e; setNotice('Registro excluído.'); await load() }
  ...
  const productOptions: Array<[string,string]> = products.map(p => [p.id, `${p.codigo} • ${p.nome}`])
  const userOptions: Array<[string,string]> = users.map(u => [u.id, `${u.nome}${u.matricula ? ` • ${u.matricula}` : ''}`])
  const competenciaOptions: Array<[string,string]> = competencias.map(c => [c.id, `${c.codigo} • ${c.nome}`])
  ...
  return <div style={{ padding: 24, maxWidth: 1600, margin: '0 auto' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(330px,420px) minmax(0,1fr)', gap: 16, alignItems: 'start' }}>
      <section style={card}>
        {tab === 'op' && <form onSubmit={submitOp}><h2 style={h2}>{editing ? 'Editar OP' : 'Nova Ordem de Produção'}</h2>{select('Produto',op.produto_id,v=>setOp(x=>({...x,produto_id:v})),productOptions)}{field('Quantidade planejada',op.quantidade_planejada,v=>setOp(x=>({...x,quantidade_planejada:v})),'number')}{field('Quantidade produzida',op.quantidade_produzida,v=>setOp(x=>({...x,quantidade_produzida:v})),'number')}{select('Status',op.status,v=>setOp(x=>({...x,status:v})),[['planejada','Planejada'],['liberada','Liberada'],['em_producao','Em produção'],['pausada','Pausada'],['concluida','Concluída'],['cancelada','Cancelada']])}{field('Data prevista',op.data_prevista,v=>setOp(x=>({...x,data_prevista:v})),'date')}{field('Observações',op.observacoes,v=>setOp(x=>({...x,observacoes:v})))}{buttons('OP')}</form>}
        ...
        {tab === 'ncr' && <form onSubmit={submitNcr}><h2 style={h2}>{editing ? 'Editar RPNC' : 'Nova Não Conformidade'}</h2>{select('Origem',ncr.origem,v=>setNcr(x=>({...x,origem:v})),[['processo','Processo'],['produto','Produto'],['cliente','Cliente'],['fornecedor','Fornecedor'],['auditoria','Auditoria']])}{select('Severidade',ncr.severidade,v=>setNcr(x=>({...x,severidade:v})),[['baixa','Baixa'],['media','Média'],['alta','Alta'],['critica','Crítica']])}{select('Produto',ncr.produto_id,v=>setNcr(x=>({...x,produto_id:v})),[['','Não informado'],...productOptions])}{field('Descrição',ncr.descricao,v=>setNcr(x=>({...x,descricao:v})))}{field('Contenção',ncr.contencao,v=>setNcr(x=>({...x,contencao:v})))}{field('Causa raiz',ncr.causa_raiz,v=>setNcr(x=>({...x,causa_raiz:v})))}{field('Prazo',ncr.prazo,v=>setNcr(x=>({...x,prazo:v})),'date')}{field('RPN',ncr.rpn,v=>setNcr(x=>({...x,rpn:v})),'number')}{select('Status',ncr.status,v=>setNcr(x=>({...x,status:v})),[['aberta','Aberta'],['contenção','Contenção'],['investigacao','Investigação'],['capa','CAPA'],['encerrada','Encerrada']])}{buttons('RPNC')}</form>}
        {tab === 'matriz' && <form onSubmit={submitMat}><h2 style={h2}>{editing ? 'Editar competência' : 'Vincular competência'}</h2>{select('Funcionário',mat.usuario_id,v=>setMat(x=>({...x,usuario_id:v})),userOptions)}{select('Competência',mat.competencia_id,v=>setMat(x=>({...x,competencia_id:v})),competenciaOptions)}{field('Nível atual',mat.nivel_atual,v=>setMat(x=>({...x,nivel_atual:v})),'number')}{select('Status',mat.status,v=>setMat(x=>({...x,status:v})),[['gap','Gap'],['em_desenvolvimento','Em desenvolvimento'],['apto','Apto'],['certificado','Certificado']])}{field('Última avaliação',mat.ultima_avaliacao,v=>setMat(x=>({...x,ultima_avaliacao:v})),'date')}{field('Próxima avaliação',mat.proxima_avaliacao,v=>setMat(x=>({...x,proxima_avaliacao:v})),'date')}{field('Observações',mat.observacoes,v=>setMat(x=>({...x,observacoes:v})))}{buttons('competência')}</form>}
        {tab === 'estoque' && <form onSubmit={submitMov}><h2 style={h2}>Novo movimento de estoque</h2><p style={{fontSize:13,color:'#64748b'}}>Movimentos são trilha operacional e não recebem exclusão por esta tela.</p>{select('Produto',mov.produto_id,v=>setMov(x=>({...x,produto_id:v})),productOptions)}{select('Tipo',mov.tipo,v=>setMov(x=>({...x,tipo:v})),[['entrada','Entrada'],['saida','Saída'],['consumo','Consumo'],['producao','Produção'],['ajuste','Ajuste'],['transferencia','Transferência'],['devolucao','Devolução']])}{field('Quantidade',mov.quantidade,v=>setMov(x=>({...x,quantidade:v})),'number')}{field('Custo unitário',mov.custo_unitario,v=>setMov(x=>({...x,custo_unitario:v})),'number')}{field('Origem',mov.origem,v=>setMov(x=>({...x,origem:v})))}{field('Observação',mov.observacao,v=>setMov(x=>({...x,observacao:v})))}{buttons('movimento')}</form>}
      </section>
