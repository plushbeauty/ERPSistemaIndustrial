...
import { Factory, Plus, Save, Trash2, X, Search, Printer, CircleDot, Paintbrush, Stamp, Boxes, ChevronRight, CheckCircle2, Image as ImageIcon, Filter } from 'lucide-react'
...
export default function FichaEngenharia(){
 ...
 if(!kind)return <main className="industrial-form-page process-sheet-page">
  ...
  <section className="industrial-panel" style={{marginBottom:16}}>
   ...
   <div className="industrial-panel" style={{marginTop:16,background:'#f7fbfc'}}><div className="process-section-heading"><span>IDENTIFICAÇÃO VISUAL</span><h2>Foto da peça / ferramental / setup</h2></div><div className="process-form-grid"><label>URL da foto principal<input value={spec.fotoPrincipal||''} onChange={e=>setS('fotoPrincipal',e.target.value)} placeholder="https://…"/></label><label>URL da segunda foto<input value={spec.fotoSecundaria||''} onChange={e=>setS('fotoSecundaria',e.target.value)} placeholder="https://…"/></label></div>{(spec.fotoPrincipal||spec.fotoSecundaria)&&<div className="grid md:grid-cols-2 gap-4" style={{marginTop:12}}>{[spec.fotoPrincipal,spec.fotoSecundaria].filter(Boolean).map((src,i)=><figure key={src} style={{margin:0}}><img src={src} alt={i?'Foto secundária da ficha':'Foto principal da ficha'} style={{width:'100%',maxHeight:260,objectFit:'contain',border:'1px solid #d7e6eb',borderRadius:12,background:'#fff'}} onError={e=>{e.currentTarget.style.display='none'}}/><figcaption><ImageIcon size={14}/> Foto {i+1}</figcaption></figure>)}</div>}</div>
  <section className="industrial-panel" style={{marginBottom:16}}><div className="process-section-heading"><span>CRIAR NOVA FICHA</span><h2>Escolha a tecnologia</h2><p>Cada modelo possui parâmetros próprios; moldes, cavidades e ferramental ficam rastreáveis.</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{kinds.map(k=>{const I=k.icon;return <button key={k.id} type="button" className="industrial-panel text-left" style={{cursor:'pointer',border:'2px solid #d7e6eb'}} onClick={()=>{setKind(k.id);setProcessName(k.title.replace('Ficha de Processo — ',''));setProcessCode('FP-'+k.id+'-001');resetForm(true,'')}}><div className="flex items-center gap-4"><span style={{display:'grid',placeItems:'center',width:64,height:64,borderRadius:16,background:'#eef7f9'}}><I size={34}/></span><div><strong style={{fontSize:20}}>{k.title}</strong><p style={{marginTop:5,color:'#5a6d75'}}>{k.description}</p></div><ChevronRight style={{marginLeft:'auto'}}/></div></button>})}</div></section>
