 ...
 * ID da Revisão: REV-066
 * Alterações: Redução efetiva dos ícones do Tablet Operacional conforme padrão compacto industrial.
...
export default function TabletLaunchpad({onNavigate,isOpen,onClose}:{onNavigate:(route:string)=>void;isOpen:boolean;onClose:()=>void}){
 ...
 return <div className="tablet-overlay" role="dialog" aria-modal="true"><section className="tablet-frame"><header className="tablet-head"><div className="tablet-brand"><img src="/logo-industrial.svg" alt=""/><div><span>SGQ ERP INDUSTRIAL</span><strong>{selected?selected.label:'Tablet Operacional'}</strong><small>{selected?selected.description:'Acesso rápido aos módulos da fábrica'}</small></div></div><div className="tablet-head-actions"><button className="tablet-icon-btn" onClick={()=>{const n=localStorage.getItem('erp-lang')==='en-US'?'pt-BR':'en-US';localStorage.setItem('erp-lang',n);location.reload()}} title="Idioma"><Languages size={17}/></button>{selected&&<button onClick={()=>setSelected(null)} className="tablet-icon-btn" title="Voltar"><ArrowLeft size={17}/></button>}<button onClick={onClose} className="tablet-icon-btn" title="Fechar tablet"><X size={17}/></button></div></header><div className="tablet-body">{!selected?<><div className="tablet-section-title"><div><span>ACESSOS PRIORITÁRIOS</span><h2>Rotinas mais usadas da fábrica</h2></div><div className="tablet-user"><UserRound size={16}/><b>Operação ERP</b></div></div><div className="tablet-grid tablet-grid-priority"><QuickCard icon={ClipboardList} title="Pedidos Pendentes" description="Carteira de vendas aguardando faturamento, produção ou expedição." onClick={()=>onNavigate('/comercial')}/><QuickCard icon={CalendarCheck2} title="PCP • Programação de Produção" description="Programação, capacidade, prioridades e sequência da produção." onClick={()=>onNavigate('/pcp')}/></div><div className="tablet-section-title"><div><span>VISUALIZAÇÃO E OPERAÇÃO</span><h2>Escolha o ambiente de trabalho</h2></div></div><div className="tablet-grid">{modules.slice(0,6).map(m=><ModuleCard key={m.label} module={m} onClick={()=>setSelected(m)}/>)}</div><div className="tablet-section-title tablet-config-title"><div><span>CONFIGURAÇÕES E APOIO</span><h2>Cadastros e administração</h2></div></div><div className="tablet-grid tablet-grid-small">{modules.slice(6).map(m=><ModuleCard key={m.label} module={m} onClick={()=>setSelected(m)}/>)}</div></>:<><button className="tablet-back" onClick={()=>setSelected(null)}><ArrowLeft size={16}/> Todos os módulos</button><div className="tablet-action-grid">{selected.actions.map(a=>{const I=a.icon;return <button key={a.label} className="tablet-action-card" onClick={()=>onNavigate(a.route)}><span className="tablet-action-icon"><I size={19}/></span><div><b>{a.label}</b><small>{a.description}</small></div><span className="tablet-open">ABRIR</span></button>})}</div></>}</div><footer className="tablet-foot"><span>© FernandoSch_System</span><span>ERP Industrial • Operação integrada • RLS</span></footer></section></div>
}
function QuickCard({icon:Icon,title,description,onClick}:{icon:IconComponent;title:string;description:string;onClick:()=>void}){return <button className="tablet-module-card tablet-priority-card" onClick={onClick}><span className="tablet-module-icon"><Icon size={24}/></span><div><b>{title}</b><small>{description}</small></div><span className="tablet-chevron">›</span></button>}
function ModuleCard({module,onClick}:{module:Module;onClick:()=>void}){const I=module.icon;return <button className="tablet-module-card" onClick={onClick}><span className="tablet-module-icon"><I size={24}/></span><div><b>{module.label}</b><small>{module.description}</small></div><span className="tablet-chevron">›</span></button>}
