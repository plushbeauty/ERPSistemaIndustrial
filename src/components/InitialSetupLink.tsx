...
export default function InitialSetupLink(){
 ...
 useEffect(()=>{let alive=true;const run=async()=>{if(!supabaseConfigurado||window.location.pathname!=='/login')return;try{const {data}=await supabase.functions.invoke('erp-login',{body:{action:'setup_status'}});if(alive)setVisible(Boolean(data?.available))}catch{if(alive)setVisible(false)}};void run();return()=>{alive=false}},[]);
 if(!visible)return null;
 return <a href="/configuracao-adm-master" style={{position:'fixed',right:18,bottom:18,zIndex:9999,display:'inline-flex',alignItems:'center',justifyContent:'center',minHeight:42,padding:'0 15px',borderRadius:10,background:'#111',border:'1px solid rgba(201,168,76,.55)',color:'#e0c56f',fontSize:11,fontWeight:900,textDecoration:'none',boxShadow:'0 12px 30px rgba(0,0,0,.3)'}}>Cadastro do proprietário Master</a>
}
