...
async function validarAcessoERP(authUserId: string): Promise<AccessResult> { try { if (!authUserId?.trim()) return { ok: false, master: false, reason: 'Sessão de autenticação inválida.' }; const { data: u, error } = await supabase.from('erp_usuarios').select('id,empresa_id,ativo,nivel_admin').eq('auth_user_id', authUserId).maybeSingle(); if (error) throw error; if (!u || u.ativo === false) return { ok: false, master: false, reason: 'Este acesso não está vinculado a um usuário ativo do ERP.' }; const master = Number(u.nivel_admin) >= 9; if (master) return { ok: true, master: true, reason: '' }; if (!u.empresa_id) return { ok: false, master: false, reason: 'Usuário sem empresa vinculada.' }; const { data: e, error: ee } = await supabase.from('erp_empresas').select('ativo,plano_status,trial_ends_at').eq('id', u.empresa_id).maybeSingle(); if (ee) throw ee; if (!e) return { ok: false, master: false, reason: 'Empresa vinculada não encontrada.' }; if (e.ativo === false) return { ok: false, master: false, reason: 'O acesso desta empresa está bloqueado.' }; if (trialExpired(e.trial_ends_at ?? null, e.plano_status ?? null)) return { ok: false, master: false, reason: 'Seu teste gratuito terminou. Ative seu plano para continuar.' }; return { ok: true, master, reason: '' } } catch (error) { console.error('[validarAcessoERP]', error); return { ok: false, master: false, reason: error instanceof Error ? error.message : 'Não foi possível validar o acesso ao ERP.' } } }
...
function Login() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (!empresa.trim() || !usuario.trim() || !pw) throw new Error('Informe empresa, usuário e senha.')

      const { data, error } = await supabase.functions.invoke('erp-login', {
        body: { empresa: empresa.trim(), identificador: usuario.trim(), senha: pw },
      })
  ...
  return <main className="auth-screen"><section className="auth-visual" aria-label="SGQ ERP Industrial"><img src="/images/sgq/sgq-erp-login.png" alt="SGQ ERP Industrial"/><div className="auth-visual-shade"/><div className="auth-visual-content"><a href="/" className="auth-visual-logo"><img src="/logo-industrial.svg" alt="SGQ ERP"/></a><div className="auth-visual-message"><span>SGQ ERP • GESTÃO INDUSTRIAL</span><h2>Uma fábrica inteira. Um único controle.</h2><p>PCP, produção, qualidade, estoque, manutenção, financeiro e fiscal trabalhando sobre os mesmos dados.</p><div className="auth-trust"><b>✓ Multiempresa</b><b>✓ Rastreabilidade</b><b>✓ Controle de acesso</b></div></div><small>FernandoSch_System</small></div></section><section className="auth-panel"><div className="auth-panel-inner"><div className="auth-mobile-brand"><a href="/"><img src="/logo-industrial.svg" alt="SGQ ERP"/></a></div><span className="auth-overline">ACESSO SEGURO</span><h1>Entrar no SGQ ERP</h1><p className="auth-description">Informe a empresa, seu usuário e sua senha. E-mail não é necessário para entrar no ERP.</p><form className="auth-form" onSubmit={submit}><label htmlFor="erp-company">Empresa</label><div className="auth-input-wrap"><input id="erp-company" type="text" value={empresa} onChange={e=>setEmpresa(e.target.value)} placeholder="Ex.: Plastibor" autoComplete="organization" required/></div><label htmlFor="erp-user">Usuário</label><div className="auth-input-wrap"><input id="erp-user" type="text" value={usuario} onChange={e=>setUsuario(e.target.value)} placeholder="Seu usuário do ERP" autoComplete="username" required/></div><label htmlFor="erp-password">Senha</label><div className="auth-input-wrap"><input id="erp-password" type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="Digite sua senha" autoComplete="current-password" required/><KeyRound size={17} aria-hidden="true"/></div>{err&&<div className="auth-message auth-error" role="alert">{err}</div>}{notice&&<div className="auth-message auth-notice" role="status">{notice}</div>}<button className="auth-submit" type="submit" disabled={busy}>{busy?'Entrando…':'Entrar'} <LogIn size={18}/></button><div className="auth-divider"><span>ou</span></div><a className="auth-register" href="/cadastro-empresa"><UserPlus size={18}/> Criar uma nova empresa</a><a className="auth-trial" href="/cadastro-empresa"><Clock3 size={17}/> Começar teste grátis de 15 dias <ArrowRight size={16}/></a></form><div className="auth-footer"><a href="/">Voltar para o site</a><span>•</span><a href="/contato">Fale conosco</a></div></div></section></main>
}
...
export default function AppEntryV2(){const[path,setPath]=useState(location.pathname),[session,setSession]=useState<Session|null>(null),[checking,setChecking]=useState(true);useEffect(()=>{let alive=true;void supabase.auth.getSession().then(({data})=>{if(alive){setSession(data.session);setChecking(false)}}).catch(error=>{console.error('[Auth bootstrap]',error);if(alive){setSession(null);setChecking(false)}});const s=supabase.auth.onAuthStateChange((_e,x)=>{if(alive)setSession(x)});return()=>{alive=false;s.data.subscription.unsubscribe()}},[]);useEffect(()=>{const f=()=>setPath(location.pathname);addEventListener('popstate',f);return()=>removeEventListener('popstate',f)},[]);if(path==='/'||path==='/home')return <Boundary><Suspense fallback={<LoadingSkeleton/>}>{session?<ERP/>:<><PublicIndustrialHome/><IndustrialVisualShowcase/></>}</Suspense></Boundary>;if(checking)return <LoadingSkeleton/>;if(path==='/login')return session?<ERP/>:<Login/>;if(path==='/cadastro-empresa')return <Boundary><Suspense fallback={lazyFallback()}><CadastroEmpresa/></Suspense></Boundary>;if(path==='/contato')return <Boundary><Suspense fallback={lazyFallback()}><Contato/></Suspense></Boundary>;if(path==='/blog')return <Boundary><Suspense fallback={lazyFallback()}><Blog/></Suspense></Boundary>;if(overviewRoutes[path])return <Boundary><Suspense fallback={lazyFallback()}>{overviewRoutes[path]}</Suspense></Boundary>;if(path==='/recebimento-materiais')return <Protected><Suspense fallback={lazyFallback()}><RecebimentoMateriais/></Suspense></Protected>;if(path==='/erp-industrial')return <Protected><ERP/></Protected>;if(path==='/master')return <Protected masterOnly><Suspense fallback={lazyFallback()}><Master/></Suspense></Protected>;if(path==='/usuarios')return <Protected><Suspense fallback={lazyFallback()}><UsuariosAdmin/></Suspense></Protected>;if(path==='/pcp')return <Protected><Suspense fallback={lazyFallback()}><PCPIndustrial/></Suspense></Protected>;if(path==='/operacao-industrial')return <Protected><Suspense fallback={lazyFallback()}><OperacaoIndustrial/></Suspense></Protected>;if(path==='/produtos-vendas')return <Protected><Suspense fallback={lazyFallback()}><ProdutosVendasIndustrial/></Suspense></Protected>;if(path==='/qualidade')return <Protected><Suspense fallback={lazyFallback()}><QualidadeIndustrial/></Suspense></Protected>;if(path==='/qualidade/documentos')return <Protected><Suspense fallback={lazyFallback()}><DocumentosQualidadeControle/></Suspense></Protected>;if(path==='/manual-usuario')return <Protected><Suspense fallback={lazyFallback()}><ManualUsuario/></Suspense></Protected>;if(path==='/compras-solicitacao')return <Protected><Suspense fallback={lazyFallback()}><SolicitacaoCompra/></Suspense></Protected>;if(path==='/fiscal')return <Protected><Suspense fallback={lazyFallback()}><Fiscal/></Suspense></Protected>;if(path==='/fiscal/previsao-caixa')return <Protected><Suspense fallback={lazyFallback()}><FiscalPrevisaoCaixa/></Suspense></Protected>;if(path==='/teste-erp')return <Protected><Suspense fallback={lazyFallback()}><TesteERP/></Suspense></Protected>;return <Boundary><Suspense fallback={<LoadingSkeleton/>}>{session?<ERP/>:<Login/>}</Suspense></Boundary>}
