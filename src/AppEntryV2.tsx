/* REVISÃO DE ENGENHARIA | Projeto: SGQ ERP Industrial | Pasta: src/ | Arquivo: AppEntryV2.tsx | Data: 2026-09-17 | Correções: sessão Auth única, validação ERP por auth_user_id, proteção de rotas, abertura do Launchpad Tablet após JWT/perfil válidos | Segurança: nenhuma senha ou chave service_role no frontend; RLS permanece ativo; nenhum bypass de autenticação | Testes/Homologação: revisão estática; build/Vercel pendente após este commit | Status: REVISADO — NÃO HOMOLOGADO */
...
function abrirLaunchpadTablet(){
 window.dispatchEvent(new CustomEvent('sgq:open-tablet'))
}
 ...
 useEffect(()=>{
  ...
  void(async()=>{
   try{
    ...
    if(error)throw error
    if(!data.session){if(alive)setValid(false);return}
    const access=await validarAcessoERP(data.session)
    if(!alive)return
    if(!access.ok){await supabase.auth.signOut();location.replace('/login');return}
    setValid(true)
    requestAnimationFrame(()=>abrirLaunchpadTablet())
   }catch(error){
    console.error('[ERP access]',error)
    if(alive){setValid(false);location.replace('/login')}
   }
  })()
  return()=>{alive=false}
 },[])
