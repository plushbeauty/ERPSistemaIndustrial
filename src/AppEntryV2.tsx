...
async function resolverLogin(login: string, empresa: string): Promise<string> {
  ...
  if (emp) {
    ...
    const match = rows.find(x => ids.has(x.empresa_id))
...
function Login() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const email = await resolverLogin(id, empresa)
      const r = await supabase.auth.signInWithPassword({ email, password: pw })
      if (r.error) {
        const msg = r.error.message.toLowerCase()
        if (msg.includes('invalid login credentials')) throw new Error('Usuário ou senha inválidos. Confira empresa, usuário e senha.')
        throw r.error
      }
  ...
  async function resetPassword() {
    ...
    setBusy(true)
    try {
      const r = await supabase.auth.resetPasswordForEmail(email, { redirectTo: `${location.origin}/login` })
  ...
  return <main className="login-page">
    ...
    <form className="login-card" onSubmit={submit}>
      <div className="login-brand">
        <a className="login-brand-logo" href="/" aria-label="Voltar para a página principal"><img className="login-logo-large" src="/logo-industrial.svg" alt="SGQ ERP" /></a>
        <h1>Acesse seu SGQ ERP</h1>
        <p>Gestão industrial integrada, segura e por empresa.</p>
      </div>
      ...
      {!recovery ? <>
        <button className="primary full login-submit" disabled={busy}>{busy ? 'Entrando…' : 'Acessar sistema'}<LogIn size={18} /></button>
        <button type="button" className="secondary full" disabled={busy} onClick={() => setRecovery(true)}>Esqueci minha senha</button>
        <a className="signup-login-link" href="/cadastro-empresa"><UserPlus size={18} /> Cadastrar nova empresa</a>
      </> : <>
        <button type="button" className="primary full" disabled={busy} onClick={() => void resetPassword()}>{busy ? 'Enviando…' : 'Enviar recuperação'}</button>
        <button type="button" className="secondary full" disabled={busy} onClick={() => setRecovery(false)}>Voltar ao login</button>
      </>}
...
function ERP() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) { if (alive) setValid(false); return }
      const access = await validarAcessoERP(data.user.id)
      if (!alive) return
      if (!access.ok) { void supabase.auth.signOut(); location.href = '/login'; return }
      setValid(true)
...
export default function AppEntryV2() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => { setSession(data.session); setChecking(false) }).catch(() => setChecking(false))
