...
class Boundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  render() {
    if (this.state.error) return <div className="error-screen"><div className="error-screen-card"><strong>Erro ao abrir a tela.</strong><p>{this.state.error.message}</p><button className="primary" onClick={() => location.reload()}>Recarregar</button></div></div>
...
function Login() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const email = await resolverLogin(id, empresa); const r = await supabase.auth.signInWithPassword({ email, password: pw })
      if (r.error) { const msg = r.error.message.toLowerCase(); if (msg.includes('invalid login credentials')) throw new Error('Usuário ou senha inválidos. Confira empresa, usuário e senha.'); throw r.error }
  ...
  return <main className="login-page">
    <section className="login-art" aria-label="Apresentação do SGQ ERP"><img src="/images/sgq/sgq-erp-login.png" alt="SGQ ERP — Gestão Industrial" /></section>
    <form className="login-card" onSubmit={submit}>
      <div className="login-brand"><a className="login-brand-logo" href="/" aria-label="Voltar para a página principal"><img className="login-logo-large" src="/logo-industrial.svg" alt="SGQ ERP" /></a><h1>Acesse seu SGQ ERP</h1><p>Gestão industrial integrada, segura e por empresa.</p></div>
      <label>Nome da Empresa<input value={empresa} onChange={e => setEmpresa(e.target.value)} placeholder="Digite o nome da sua empresa" autoComplete="organization" required /></label>
      <label>Usuário ou e-mail<input value={id} onChange={e => setId(e.target.value)} placeholder="Digite seu usuário ou e-mail" autoComplete="username" required /></label>
...
function ERP() {
  ...
  if (valid === null) return <div className="loading-screen">Validando acesso…</div>; if (valid === false) return <Login/>; return <AppIndustrial/>
}
