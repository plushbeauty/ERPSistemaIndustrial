...
function Module({children,label='Abrindo módulo…'}:{children:ReactNode;label?:string}){
  return <Boundary><Suspense fallback={<LoadingSkeleton label={label}/>}>{children}</Suspense></Boundary>
}
...
function ERP(){
  ...
  return <Module label="Abrindo painel industrial…"><><AppIndustrial/><VirtualGuide brand="SGQ ERP" name="Dri"/></></Module>
}
...
export default function AppEntryV2(){
  ...
  useEffect(()=>{if(path==='/'){location.replace('/login')}},[path])
  if(path==='/')return <LoadingSkeleton label="Abrindo login…"/>
  if(checking)return <LoadingSkeleton/>
  if(path==='/login')return session?<ERP/>:<Login/>
  if(path==='/blog')return <Module label="Abrindo conteúdos…"><Blog/></Module>
  if(path==='/cadastro-empresa')return <Module label="Abrindo cadastro…"><CadastroEmpresa/></Module>
  if(path==='/erp-industrial')return <Protected><Module label="Abrindo ERP industrial…"><ERP/></Module></Protected>
  if(path==='/master')return <Protected masterOnly><Module label="Abrindo painel master…"><Master/></Module></Protected>
  if(path==='/usuarios')return <Protected><Module><UsuariosAdmin/></Module></Protected>
  if(path==='/pcp')return <Protected><Module><PCPIndustrial/></Module></Protected>
  if(path==='/operacao-industrial')return <Protected><Module><OperacaoIndustrial/></Module></Protected>
  if(path==='/produtos-vendas')return <Protected><Module><ProdutosVendasIndustrial/></Module></Protected>
  if(path==='/qualidade')return <Protected><Module><QualidadeIndustrial/></Module></Protected>
  if(path==='/qualidade/documentos')return <Protected><Module><DocumentosQualidadeControle/></Module></Protected>
  if(path==='/manual-usuario')return <Protected><Module><ManualUsuario/></Module></Protected>
  if(path==='/compras-solicitacao')return <Protected><Module><SolicitacaoCompra/></Module></Protected>
  if(path==='/fiscal')return <Protected><Module><Fiscal/></Module></Protected>
  if(path==='/fiscal/previsao-caixa')return <Protected><Module><FiscalPrevisaoCaixa/></Module></Protected>
  if(path==='/teste-erp')return <Protected><Module><TesteERP/></Module></Protected>
  return <Module><PublicIndustrialHome/></Module>
}
