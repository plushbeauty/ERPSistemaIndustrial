...
function Login() {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); setErr(''); setNotice(''); setBusy(true); try { if (!supabaseConfigurado) throw new Error('A conexão do sistema com o banco não está configurada.'); if (!email.trim() || !pw) throw new Error('Informe e-mail e senha.'); const { data, error } = await supabase.auth.signInWithPassword({ email: email.trim().toLowerCase(), password: pw }); if (error) throw error; if (!data.user) throw new Error('O Supabase Auth não retornou o usuário autenticado.'); const access = await validarAcessoERP(data.user.id); if (!access.ok) { await supabase.auth.signOut(); throw new Error(access.reason) } location.replace(requestedTarget()) } catch (e) { setErr(e instanceof Error ? e.message : 'Não foi possível entrar no sistema.') } finally { setBusy(false) } }
  async function resetPassword() { setErr(''); setNotice(''); const target = email.trim().toLowerCase(); if (!target || !target.includes('@')) { setErr('Informe o e-mail cadastrado para recuperar a senha.'); return } setBusy(true); try { const { error } = await supabase.auth.resetPasswordForEmail(target, { redirectTo: `${location.origin}/login` }); if (error) throw error; setNotice('Se o e-mail estiver cadastrado, o link de recuperação será enviado.'); setRecovery(false) } catch (e) { setErr(e instanceof Error ? e.message : 'Não foi possível solicitar a recuperação.') } finally { setBusy(false) } }
