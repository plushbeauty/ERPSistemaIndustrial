...
export default function AppIndustrialV7() {
  ...
  return <motion.div className={`v7-shell${dark ? " theme-dark" : ""}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.28 }}>
    <aside className="v7-sidebar"><div className="v7-brand"><img src="/logo-industrial.svg" alt="SGQ ERP"/><div><strong>SGQ ERP</strong><span>ERP Multissegmento</span></div></div><button className="v7-all" onClick={() => setLauncher(true)}><LayoutGrid size={18}/> Todos os módulos</button><div className="v7-segment-label">SEGMENTO ATIVO</div><button className="v7-segment-current" onClick={() => setLauncher(true)}><Factory size={17}/><span>{segment}</span><span>⌄</span></button><nav>{current.modules.map(m => <Nav key={m.name} module={m} active={active} setActive={setActive}/>)}</nav><button className={active === 'Configurações' ? 'v7-nav active' : 'v7-nav'} onClick={() => setActive('Configurações')}><Settings size={17}/>Configurações</button><button className="v7-exit" onClick={() => void supabase.auth.signOut().then(() => { location.href = '/login' })}>Sair</button></aside>
    <main className="v7-main"><header className="v7-header"><div><span>SGQ ERP • {segment.toUpperCase()}</span><h1>{active === 'Dashboard' ? `Dashboard — ${segment}` : module?.title ?? active}</h1><p>{active === 'Dashboard' ? current.description : module?.description}</p></div><div className="v7-user"><b>{profile.nome}</b><small>Empresa {profile.empresa_id}</small><button className="v7-theme-toggle" type="button" onClick={() => setDark(v => !v)} aria-label={dark ? "Ativar modo claro" : "Ativar modo escuro"}>{dark ? <Sun size={16}/> : <Moon size={16}/>} {dark ? "Claro" : "Escuro"}</button></div></header><section className="v7-content"><AnimatePresence mode="wait" initial={false}><motion.div key={active} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-5}} transition={{duration:.2}}>{active === 'Dashboard' ? <Dashboard segment={current} setActive={setActive}/> : active === 'Configurações' ? <Feature title="Configurações" description="Parâmetros, usuários, permissões e módulos são separados por empresa e segmento." icon={Settings}/> : module?.table ? <Crud module={module} profile={profile}/> : <Feature title={module?.title ?? active} description={module?.description ?? 'Módulo preparado para este segmento.'} icon={module?.icon ?? LayoutGrid}/>}</motion.div></AnimatePresence></section><footer>FernandoSch_System • SGQ ERP • {segment} • Ambiente isolado por empresa</footer></main>
    {launcher && <div className="v7-launcher-backdrop" onMouseDown={() => setLauncher(false)}><div className="v7-launcher" role="dialog" aria-modal="true" onMouseDown={e => e.stopPropagation()}><header><div><strong>Todos os módulos</strong><span>Escolha o segmento e o módulo operacional</span></div><button onClick={() => setLauncher(false)} aria-label="Fechar"><X size={20}/></button></header><div className="v7-segment-grid">{segments.map(s => { const Icon = s.icon; return <article key={s.name} className={s.name === segment ? 'v7-segment-card selected' : 'v7-segment-card'}><div className="v7-segment-card-head"><span className="v7-icon-box"><Icon size={20}/></span><div><b>{s.name}</b><small>{s.description}</small></div></div><div className="v7-module-grid">{s.modules.map(m => { const I = m.icon; return <button key={m.name} onClick={() => choose(s, m.name)}><I size={16}/><span>{m.title}</span></button> })}</div></article> })}</div></div></div>}
  </motion.div>
}
