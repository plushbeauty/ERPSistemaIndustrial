...
export default function AppIndustrialV7() {
  ...
  return <div className="v7-shell">
    ...
    <main className="v7-main"><header className="v7-header"><div><span>SGQ ERP • {segment.toUpperCase()}</span><h1>{active === 'Dashboard' ? `Dashboard — ${segment}` : module?.title ?? active}</h1><p>{active === 'Dashboard' ? current.description : module?.description}</p></div><div className="v7-user"><b>{profile.nome}</b><small>Empresa {profile.empresa_id}</small></div></header><section className="v7-content">{active === 'Dashboard' ? <Dashboard segment={current} setActive={setActive}/> : active === 'Configurações' ? <Feature title="Configurações" description="Parâmetros, usuários, permissões e módulos são separados por empresa e segmento." icon={Settings}/> : module?.table ? <Crud module={module} profile={profile}/> : <Feature title={module?.title ?? active} description={module?.description ?? 'Módulo preparado para este segmento.'} icon={module?.icon ?? LayoutGrid}/>}</section><footer>FernandoSch_System • SGQ ERP • {segment} • Ambiente isolado por empresa</footer></main>
