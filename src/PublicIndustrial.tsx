...
const modules = [
  ['Produção', Factory, 'Ordens de produção, processos, apontamentos e rastreabilidade.', 'pcp'],
  ['Estoque / WMS', Package, 'Materiais, produtos, lotes, validade e disponibilidade.', 'estoque'],
  ['MRP / PCP', Gauge, 'Planejamento de necessidades, capacidade e agenda de máquinas.', 'pcp'],
  ['Qualidade', ShieldCheck, 'RPNC, ações corretivas, inspeções, evidências e histórico.', 'qualidade'],
  ['Manutenção', Wrench, 'Máquinas, ordens, preventivas e histórico de manutenção.', 'manutencao'],
  ['Compras / Vendas', ShoppingCart, 'Fornecedores, clientes, pedidos, vendas e fluxo comercial.', 'compras'],
]
  ...
  return <div className="public-industrial">
    ...
    <main>
      <section className="public-hero">
        ...
        <div className="dashboard-preview real-module-preview"><div className="preview-top"><div><span>AMBIENTE INDUSTRIAL</span><strong>Módulos conectados ao ERP</strong></div><span className="preview-status">● Dados reais após login</span></div><div className="preview-grid"><a href="/login"><Factory size={16}/> Produção e OP</a><a href="/login"><Boxes size={16}/> Estoque e materiais</a><a href="/login"><Gauge size={16}/> PCP e MRP</a><a href="/login"><ShieldCheck size={16}/> Qualidade e RPNC</a></div><div className="preview-grid"><a href="/login"><Wrench size={16}/> Manutenção</a><a href="/login"><ShoppingCart size={16}/> Compras e vendas</a><a href="/login"><Package size={16}/> Rastreabilidade</a><a href="/login"><BarChart3 size={16}/> Indicadores</a></div><p className="preview-real-data-note">Os indicadores operacionais são carregados do banco somente dentro do ambiente autenticado. A página pública não apresenta números simulados.</p></div>
      </section>

      <section id="recursos" className="public-section"><div className="section-heading"><span className="public-kicker">GESTÃO INTEGRADA</span><h2>Do pedido ao produto acabado.</h2><p>Conheça cada módulo antes de entrar no sistema.</p></div><div className="module-cards">{modules.map(([name, Icon, text, key]) => <article className="module-card" key={String(name)}><div className="module-icon"><Icon size={22} /></div><h3>{String(name)}</h3><p>{String(text)}</p><a href={`/modulos/${String(key)}`}>Ver o que este módulo faz <ArrowRight size={15}/></a></article>)}</div></section>

      <section id="por-dentro" className="public-section inside-section"><div className="section-heading"><span className="public-kicker">POR DENTRO DO SISTEMA</span><h2>Entre no ambiente operacional real.</h2><p>Depois do login, cada área consulta os dados autorizados da sua empresa e registra operações no ERP.</p></div><div className="inside-grid"><article className="screen-card"><div className="screen-bar"><b>PCP / MRP</b><span>Ambiente autenticado</span></div><div className="screen-body"><p>Planejamento, necessidades de materiais, capacidade e programação vinculados às ordens e cadastros reais.</p><a href="/login">Acessar ERP <ArrowRight size={15}/></a></div></article><article className="screen-card"><div className="screen-bar"><b>Produção / MES</b><span>Ambiente autenticado</span></div><div className="screen-body"><p>Apontamentos, operações, paradas, lotes, séries e rastreabilidade registrados no banco.</p><a href="/login">Acessar ERP <ArrowRight size={15}/></a></div></article><article className="screen-card"><div className="screen-bar"><b>Estoque / WMS</b><span>Ambiente autenticado</span></div><div className="screen-body"><p>Movimentações, reservas, endereçamento, inventário e saldos calculados a partir das transações.</p><a href="/login">Acessar ERP <ArrowRight size={15}/></a></div></article><article className="screen-card"><div className="screen-bar"><b>Qualidade / SGQ</b><span>Ambiente autenticado</span></div><div className="screen-body"><p>Inspeções, não conformidades, ações, evidências e rastreabilidade conforme as permissões do usuário.</p><a href="/login">Acessar ERP <ArrowRight size={15}/></a></div></article></div></section>
